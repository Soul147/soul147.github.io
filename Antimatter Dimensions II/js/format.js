function getFullExpansion(num, places) {
	...
	if(num.gt(1e15)) return shorten(num, places);
	var decimal = num.subtract(num.floor())
	num = num.floor()
	return num.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + decimal.toFixed(Math.max(places - num.log10(), 0)).slice(1)
}
