function updateDimensionSet(name="dimension", abbr="", curr="") {
	...
	for(var i = 10; i >= 0; i--) {
		if(i < 10-c12) {
			...
			if(c12) var dimProductionUp = window["get" + Name + "Production"](i + 2).multiply(tickspeed).multiply(getChallengeDivider(name))
...
function update() {
	...
	c = game.dimensions[0].amount.gte(infp()) && !(game.bestInfinityTime < 60000 || game.break);
	...
	ge("repeatInf0").innerHTML = "Multiply IP gain by 2<br>Currently: " + shortenMoney(Decimal.pow(2, game.repeatInf[0].bought)) + "x<br>Cost: " + shortenMoney(getRepeatInfCost(0)) + " IP"
	// ge("repeatInf1").innerHTML = "Tickspeed cost multiplier increase<br>" + game.tickCostMultIncrease + "x" + (game.repeatInf[1].bought.lt(7) ? " > " + (game.tickCostMultIncrease-1) + "x<br>Cost: " + shortenMoney(getRepeatInfCost(1)) + " IP" : "")
	...
	ge("repeatInf1").className = game.repeatInf[1].bought.gt(6) ? "infinityUpgradeBought" : canBuyRepeatInf(1) ? "infinityUpgrade" : "infinityUpgradeLocked"
	...
	ge("infinityshiftcost").textContent = (getChallengeCompletions(1) > 5 || game.infinityShifts.lt(4)) ? "Reach " + shortenCosts(getInfinityShiftCost()) + " antimatter to unlock a new dimension." : "Complete 6 infinity challenges to unlock."
	...
	if(game.dimensions[8].multiplier.gt(infp())) giveAchievement(53)
