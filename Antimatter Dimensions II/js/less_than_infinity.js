;(function (globalScope) {
	'use strict';	
	/*
		# less_than_infinity.js
		A simple attempt at storing unnecessarily large numbers, without going to break_eternity levels.
		It's basically confractus_numerus.js but, you know, it works. It should also in theory be faster.
		Based off of logarithmica_numerus but the logarithm variable is a logarithm itself.

		This library uses SmallDecimal which is expressed as 10^(logarithm). There is only a factor:
	...
	*/
	class SmallDecimal {
		static fromValue(v) {
			return new SmallDecimal(v)
		}
		
		constructor(v) { 
			if (v==null) {
				this.l=Number.NEGATIVE_INFINITY
			} else if (v instanceof SmallDecimal) {
				this.l=v.l
		...
		static fromMantissaExponent(m,e) {
			var v=new SmallDecimal()
			v.l=e+Math.log10(m)
			return v
		}
		
		static fromMantissaExponent_noNormalize(m,e) {
			return SmallDecimal.fromMantissaExponent(m,e)
		}
		
		static toString(v) {
			v=new SmallDecimal(v)
		...
		toString() {
			return SmallDecimal.toString(this)
		}
		...
		toNumber() {
			return SmallDecimal.toNumber(this)
		}
		...
		toPrecision(dp) {
			return SmallDecimal.toPrecision(this,dp)
		}
		...
		toFixed(dp) {
			return SmallDecimal.toFixed(this,dp)
		}
		...
		toExponential(dp) {
			return SmallDecimal.toExponential(this,dp)
		}
		
		static add(v1,v2) {
			v1=new SmallDecimal(v1)
			v2=new SmallDecimal(v2)
		...
		add(v) {
			return SmallDecimal.add(this,v)
		}
		
		static plus(v1,v2) {
			return SmallDecimal.add(v1,v2)
		}
		...
		sub(v) {
			return SmallDecimal.sub(this,v)
		}
		
		static subtract(v1,v2) {
			return SmallDecimal.sub(v1,v2)
		}
		...
		mul(v) {
			return SmallDecimal.mul(this,v)
		}
		
		static multiply(v1,v2) {
			return SmallDecimal.mul(v1,v2)
		}
		...
		div(v) {
			return SmallDecimal.div(this,v)
		}
		
		static divide(v1,v2) {
			return SmallDecimal.div(v1,v2)
		}
		...
		recip() {
			return SmallDecimal.recip(this)
		}
		
		static reciprocal(v) {
			return SmallDecimal.recip(v)
		}
		...
		mod(v) {
			return SmallDecimal.mod(this,v)
		}
		
		static remainder(v1,v2) {
			return SmallDecimal.mod(v1,v2)
		}
		...
		pow(v) {
			return SmallDecimal.pow(this,v)
		}
		
		static power(v,power) {
			return SmallDecimal.pow(v,power)
		}
		...
		pow_b(v) {
			return SmallDecimal.pow(v,this)
		}
		...
		sqr() {
			return SmallDecimal.square(this)
		}
		
		static square(v) {
			return SmallDecimal.sqr(v)
		}
		
		square() {
			return SmallDecimal.sqr(this)
		}
		...
		cub() {
			return SmallDecimal.cube(this)
		}
		
		static cube(v) {
			return SmallDecimal.cub(v)
		}
		
		cube() {
			return SmallDecimal.cub(this)
		}
		...
		root(v) {
			return SmallDecimal.root(this,v)
		}
		...
		log10integer() {
			return SmallDecimal.log10integer(this)
		}
		...
		log10remainder() {
			return SmallDecimal.log10remainder(this)
		}
		...
		log2() {
			return SmallDecimal.log2(this)
		}
		
		static log(v,b) {
			v=new SmallDecimal(v)
			b=new SmallDecimal(b)
			return v.l/b.l
		}
		
		log(b) {
			return SmallDecimal.log(this,b)
		}
		
		static l(v,b) {
			return SmallDecimal.log(v,b)
		}
		...
		floor() {
			return SmallDecimal.floor(this)
		}
		...
		round() {
			return SmallDecimal.round(this)
		}
		...
		min(v) {
			return SmallDecimal.min(this,v)
		}
		...
		max(v) {
			return SmallDecimal.max(this,v)
		}
		...
		compareTo(v) {
			return SmallDecimal.cmp(this,v)
		}
		
		static compare(v1,v2) {
			return SmallDecimal.cmp(v1,v2)
		}
		...
		lte(v) {
			return SmallDecimal.lte(this,v)
		}
		...
		eq(v) {
			return SmallDecimal.eq(this,v)
		}
		
		static equals(v1,v2) {
			return SmallDecimal.eq(v1,v2)
		}
		...
		neq(v) {
			return SmallDecimal.neq(this,v)
		}
		
		static notEquals(v1,v2) {
			return SmallDecimal.neq(v1,v2)
		}
		...
		gte(v) {
			return SmallDecimal.gte(this,v)
		}
		...
		isFinite() {
			return SmallDecimal.isFinite(this)
		}
	...
	var noConflict = globalScope.SmallDecimal;
	SmallDecimal.noConflict = function () {
		globalScope.SmallDecimal = noConflict;
		return SmallDecimal;
	};

	globalScope.SmallDecimal = SmallDecimal;
	}
	...
	class Decimal {
		...
		static fromString(v) {
			...
			if (findE==-1) {
				v=parseFloat(v)
				tmp.l=SmallDecimal.log10(v)
			} else {
				var split=[v.slice(0,findE),v.slice(findE+1,v.length)]
				split[1]=parseFloat(split[1])
				if (split[0]=='') tmp.l = new SmallDecimal(split[1])
				else {
					split[0]=parseFloat(split[0])
					tmp.l=SmallDecimal.log10(split[0]).add(split[1])
				}
			}
		...
		static fromMantissaExponent(m,e) {
			var v=new Decimal()
			v.l=SmallDecimal.add(e, Math.log10(m))
			return v
		}
		...
		static toString(v) {
			v=new Decimal(v)
			if (v.l.eq(Number.NEGATIVE_INFINITY) || v.l.l == Number.NEGATIVE_INFINITY) return '0'
			if (v.l.eq(Number.POSITIVE_INFINITY) || v.l.l == Number.POSITIVE_INFINITY) {
				return 'Infinity'
			}
			if (v.l.gte(1e21)||v.l.lte(-1e21)) {
				return 'e'+v.l
			}
			if (v.l.gte(21)||v.l.lt(-6)) {
				var logInt=SmallDecimal.floor(v.l)
				return SmallDecimal.pow(10,v.l.subtract(logInt))+'e'+logInt
			}
			return Decimal.pow(10,v.l).toString()
		}
