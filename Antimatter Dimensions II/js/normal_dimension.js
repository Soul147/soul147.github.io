...
function maxDimension(i, b) {
	...
	if(dim.cost.lte(Number.MAX_VALUE)) {
		let temp = game.dimensions[0].amount.min(Number.MAX_VALUE).log10().subtract(dim.cost.log10())
		var bought = temp.eq(0)?new Decimal(1):temp.divide(dim.costMult.log10()).ceil()

		dim.amount = dim.amount.add(bought);
		dim.bought = dim.bought.add(bought);
		game.dimensions[0].amount = game.dimensions[0].amount.minus(dim.cost.multiply(dim.costMult.pow(bought.minus(1))))
